...
<head>
  ...
  <title>Midnight Mainnet Explorer</title>
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .mainnet-badge { display: inline-block; background: #ff6b35; color: #000; padding: 3px 10px; border-radius: 4px; font-size: 12px; font-weight: bold; margin-left: 10px; }
    .stats { display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 15px; margin-bottom: 30px; }
    .stat { background: #1a1a2e; padding: 20px; border-radius: 10px; border: 1px solid #3a2a1e; }
    .stat-label { color: #888; font-size: 12px; text-transform: uppercase; }
    .stat-value { font-size: 24px; font-weight: bold; color: #ff6b35; margin-top: 5px; }
    .section { background: #1a1a2e; border-radius: 10px; padding: 20px; margin-bottom: 20px; border: 1px solid #3a2a1e; }
    .section h2 { color: #ff6b35; margin-bottom: 15px; font-size: 18px; }
    ...
    .hash { color: #ff8c5a; max-width: 200px; overflow: hidden; text-overflow: ellipsis; }
    a { color: #ff6b35; text-decoration: none; }
    a:hover { text-decoration: underline; }
    .search { margin-bottom: 30px; display: flex; gap: 10px; }
    .search input { flex: 1; padding: 12px 15px; border: 1px solid #3a2a1e; border-radius: 8px; background: #1a1a2e; color: #e0e0e0; font-size: 14px; }
    .search button { padding: 12px 25px; background: #ff6b35; border: none; border-radius: 8px; color: white; cursor: pointer; font-weight: bold; }
    .search button:hover { background: #e55a2b; }
    ...
    .epoch-value { font-family: monospace; color: #ff8c5a; }
  </style>
</head>
<body>
  <div class="container">
    <h1>Midnight Mainnet Explorer <span class="mainnet-badge">MAINNET</span></h1>
    <p class="subtitle">Block explorer for Midnight Mainnet (Production) - Port 3005</p>
    ...
    <div class="section">
      <h2>Network Info</h2>
      <table>
        <tr><td>Network</td><td>${config.network.name} <span class="mainnet-badge">PRODUCTION</span></td></tr>
        <tr><td>RPC Endpoint</td><td><code>${config.network.rpcEndpoint}</code></td></tr>
        <tr><td>Genesis Hash</td><td class="hash">${config.network.genesisHash || 'Connecting...'}</td></tr>
        <tr><td>Node Version</td><td>${config.node.version || 'Connecting...'}</td></tr>
        <tr><td>Cardano Network</td><td>Mainnet</td></tr>
      </table>
    </div>
...
<body class="bg-dark-400 text-gray-200 min-h-screen flex">
    ...
    <nav class="p-4 space-y-4 text-sm">
      <div>
        ...
        <ul class="space-y-1">
          ...
          <li><a href="#price" class="block px-2 py-1 rounded hover:bg-dark-100 text-slate-300 hover:text-white">/api/price</a></li>
        </ul>
      </div>
      <div>
  ...
  { "address": "5G...", "last_seen": 1711882800, "blocks_produced": 500 }
        ...
        <div id="price" class="mb-8 bg-dark-200 rounded-lg border border-slate-800 p-5">
          <div class="flex items-center justify-between mb-3">
            <div class="flex items-center gap-2">
              <span class="bg-emerald-700 text-white text-xs font-bold px-2 py-0.5 rounded">GET</span>
              <code class="text-white">/api/price</code>
            </div>
            <div class="flex gap-2">
              <button onclick="copyUrl('/api/price')" class="copy-btn text-xs bg-dark-100 hover:bg-slate-700 text-slate-400 px-2 py-1 rounded">Copy URL</button>
              <a href="${baseUrl}/api/price" target="_blank" class="text-xs bg-midnight-500/20 hover:bg-midnight-500/30 text-midnight-400 px-2 py-1 rounded">Try it</a>
            </div>
          </div>
          <p class="text-sm text-slate-400 mb-3">CoinGecko price proxy with 5-minute cache. Defaults to NIGHT/USD.</p>
          <pre class="bg-dark-400 rounded p-3 text-xs text-slate-300 overflow-x-auto"><code>{ "midnight-3": { "usd": 0.025 } }</code></pre>
        </div>
      </section>
      ...
      <section class="mb-12">
        ...
        <div id="overview" class="mb-8 bg-dark-200 rounded-lg border border-slate-800 p-5">
          ...
          <pre class="bg-dark-400 rounded p-3 text-xs text-slate-300 overflow-x-auto"><code>{
  "network": "Midnight Mainnet", "blocks": 150000, "extrinsics": 500000,
  ...
  { "hour": "2026-03-31 12:00:00", "count": 150 }
      ...
      <section class="mb-12">
        ...
        <div id="privacy" class="mb-8 bg-dark-200 rounded-lg border border-slate-800 p-5">
          ...
          <p class="text-sm text-slate-400 mb-1">Privacy dashboard: shielded/unshielded ratios, contract deploys/calls, and trends (event-based analysis).</p>
    ...
    "signer": "5G...", "shieldingType": "shielded", "txType": "transfer" }
      ...
      <section class="mb-12">
        ...
        <div id="bridge" class="mb-8 bg-dark-200 rounded-lg border border-slate-800 p-5">
          ...
          <pre class="bg-dark-400 rounded p-3 text-xs text-slate-300 overflow-x-auto"><code>{
  "totalBridgeOps": 5000, "last24h": 120,
  "trend": [...],
  "recentBridgeOps": [
    ...
    "block_height": 149000, "timestamp": 1711880000 }
      ...
      <section class="mb-12">
        ...
        <div id="cardano-anchors" class="mb-8 bg-dark-200 rounded-lg border border-slate-800 p-5">
          ...
          <p class="text-sm text-slate-400 mb-1">Cardano anchor points: sidechain state anchored to Cardano mainchain.</p>
          <p class="text-xs text-slate-500 mb-3">Params: <code class="text-slate-400">limit</code> (max 200, default 50)</p>
          <pre class="bg-dark-400 rounded p-3 text-xs text-slate-300 overflow-x-auto"><code>[
